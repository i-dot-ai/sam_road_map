...
class GraphLabelGenerator:
    def __init__(self, config, full_graph, coord_transform):
        ...
        self.full_graph_origin = graph_utils.igraph_from_adj_dict(
            full_graph, coord_transform, dataset=config.DATASET
        )
        ...
        self.nms_score_override[
            np.array(list(itsc_indices))
...
def test_graph_label_generator(rgb_path, gt_graph):
...
class SatMapDataset(Dataset):
    def __init__(
        self, config, is_train, dev_run=False, return_graph=True, return_metadata=False
    ):
        ...
        if hasattr(data_config, "metadata") and data_config.metadata.get(
            "is_composite", False
        ):
        ...
        if (
            ...
            or len(dataset_handler.get_all_tile_ids()) == 0
        ):
            print(
                f"Dataset {dataset_id} not found locally, attempting to download from GCP..."
            )
        ...
        metadata_pattern = (
            f"{dataset_dir}/{{}}/metadata.json"  # Pattern for metadata files
        )
            ...
            try:
                ...
                if self.return_metadata:
                    try:
                        ...
                        print(
                            f"===== warning: no valid metadata for tile {tile_idx} ====="
                        )
        ...
        if self.is_composite:
            ...
            logging.info(
                f"Class distribution in {dataset_id}: {self.class_distribution}"
            )
    ...
    def add_offset_center(
        self, metadata, offset_distance_meters=None, direction_degrees=None
    ):
        """
    ...
    def calculate_sample_weights(self):
        ...
        total_samples = sum(self.class_distribution.values())
        class_weights = {
            cls: total_samples / count for cls, count in self.class_distribution.items()
        }

        # Create sample weights array
        sample_weights = []
        for i in range(len(self)):
            tile_idx = self.tile_indices[
                i % len(self.tile_indices)
            ]  # Handle case when __len__ returns a different value
    ...
    def __getitem__(self, idx):
        ...
        if self.return_graph:
            ...
            graph_points, topo_samples = self.graph_label_generators[
                img_idx
            ].sample_patch(patch, rot_index)
