...
def generate_data_split(
    data_dir, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, seed=42
):
    """
    ...
    for file in os.listdir(data_dir):
        if (
            file.endswith(".png")
            and not file.endswith("_keypoints.png")
            and not file.endswith("_road_mask.png")
        ):
    ...
    print(
        f"Split created with {len(train_files)} train, {len(val_files)} validation, and {len(test_files)} test samples"
    )
