...
def evaluate_georeferencer(
    config, output_dir, zoom=16, num_samples=None, debug_mode=True
):
    """
    ...
    val_ds = SatMapDataset(
        config, is_train=False, return_graph=False, return_metadata=True
    )
    logger.info(f"Dataset length: {len(val_ds)}")

    # Initialize georeferencer
    logger.info("Initializing georeferencer...")
    georeferencer = RoadMatcherGeoreferencer(
        sam_config_path=config.__config_path__,
    ...
    total_samples = (
        len(val_ds) if num_samples is None else min(num_samples, len(val_ds))
    )
    ...
    for i in tqdm(range(total_samples)):
        try:
            ...
            try:
                new_point = georeferencer.georeference(
                    datum["rgb"],
                    offset_point,
                    zoom=zoom,
                    debug_ouput_subfolder=f"sample_{i}",
                ).tolist()
            except ValueError as ve:
                logger.warning(
                    f"ValueError in sample {i} during georeferencing: {str(ve)}"
                )
    ...
    results = {"statistics": stats, "samples": sample_data}
...
def generate_visualizations(results, df, output_dir):
    ...
    plt.savefig(
        os.path.join(vis_dir, "error_distribution.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.title(
        f'Offset Distance vs. Error (Correlation: {stats["offset_error_correlation"]:.3f})'
    )
    plt.grid(True, alpha=0.3)
    plt.savefig(
        os.path.join(vis_dir, "offset_vs_error.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    for p, color, label in [
        (0.5, "r", f"50% < {np.median(distances):.1f}m"),
...
def generate_report(results, output_dir):
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            f"- Correlation between offset and error: {stats['offset_error_correlation']:.3f}\n\n"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate the RoadMatcherGeoreferencer"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    output_dir = create_output_dir_and_save_config(
        output_prefix, config, args.output_dir
    )
    ...
    logger.info(
        f"Evaluation complete. Report saved to: {os.path.join(output_dir, 'report.md')}"
    )
